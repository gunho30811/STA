...
.tog.on{color:#111827}
.chip{color:#475569}
.chip.on{background:#1B72E8;border-color:#1B72E8;color:#fff}
...
.sbox button{border:none;background:#1B72E8;color:#fff;border-radius:999px;padding:5px 11px;font-size:12px;font-weight:700;cursor:pointer}
...
.poi.h{border-color:#dc2626;color:#991b1b}.poi.u{border-color:#2563eb;color:#1e40af}.poi.i{border-color:#1B72E8;color:#5b21b6}
...
background:rgba(27,114,232,.93);color:#fff;border:2.5px solid rgba(255,255,255,.9);border-radius:13px;
...
.it-go{font-size:11px;color:#1B72E8;font-weight:700;white-space:nowrap}
...
.ri:focus{outline:none;border-color:#1B72E8}
...
.rbtn{width:100%;margin-top:16px;padding:12px;border:none;border-radius:10px;background:#1B72E8;color:#fff;
...
#demobar a{background:#1B72E8;color:#fff;font-weight:800;padding:8px 18px;border-radius:8px;
...
<div class=bar>
  ...
  <div class=row>
    ...
    <label class="tog" id=t_rent><span class=dot style="background:#1B72E8"></span>렌트</label>
      ...
      <div class=rrow>
        ...
        <div style="display:flex;align-items:flex-end;padding-bottom:9px">
          <label style="font-size:12.5px;font-weight:700;color:#475569;display:flex;align-items:center;gap:6px;cursor:pointer">
            <input type=checkbox id=r_only checked style="width:16px;height:16px;accent-color:#1B72E8"> 조건 맞는 매물 있는 동만</label>
        </div>
      </div>
...
function signupNudge(what){
  ...
  b.innerHTML='<div style="padding:18px 10px;text-align:center">'+
    ...
    '<a href="/auth/signup" style="display:inline-block;background:#1B72E8;color:#fff;font-weight:800;'+
...
function geoStyle(i){
  ...
  if(occ>=70) return {c:'#1B72E8',w:1,fc:'#1B72E8',fo:.32}
  if(occ>=55) return {c:'#6d5ef2',w:.8,fc:'#6d5ef2',fo:.20}
  if(occ>=40) return {c:'#8FC0FA',w:.6,fc:'#8FC0FA',fo:.11}
