...
def init_auth(app, demo_endpoints=None):
    ...
    def _guard():
        ...
        if (ep.startswith("auth.") or ep in ("static", "home", "og_image", "dong_geo",
                                             "chat_api_cron_poll", "api_cron_alerts")):
            return None
        if not session.get("uid"):
...
@media(max-width:900px){{
  ...
  #__nav .__menu{{display:none;position:absolute;top:100%;left:0;right:0;background:#fff;
    ...
    border-bottom:1px solid #EEF1F6;box-shadow:0 8px 20px rgba(27,27,58,.08)}}
  ...
  #__nav .__right{{margin-left:0;flex-direction:column;align-items:stretch;
    border-top:1px solid #EEF1F6;margin-top:8px;padding-top:8px}}
}}
</style>
<script>
