...
export default function App() {
  ...
  return (
    <>
      <header>
        <h1>💰 단기임대 수익성</h1>
        <p>렌트 단기임대 풀가동 시 부동산 장기월세 대비 최대수익·순수익 + 동/역 수요(예약률) · 단위 만원 · 행 클릭 시 오른쪽 상세</p>
      </header>
      ...
      {demo && (
        <div style={{ background: '#1e293b', color: '#e2e8f0', padding: '10px 22px', fontSize: 13.5, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
          ...
          <a href="/auth/signup" style={{ background: '#1B72E8', color: '#fff', textDecoration: 'none', fontWeight: 800, padding: '7px 16px', borderRadius: 8 }}>무료 회원가입 →</a>
        </div>
      )}
      ...
      <div className="wrap">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '2px 0 12px', flexWrap: 'wrap' }}>
          ...
          <span className="legend hide-mobile" style={{ margin: 0 }}>
            {months.length
              ? '달력월별(예: 2026-08 예약률)은 렌트 재크롤분부터 채워져요.'
              : '아직 달력월별 데이터가 없어요 — 렌트 재크롤 후 채워집니다.'}
          </span>
        </div>
