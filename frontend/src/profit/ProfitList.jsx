...
const COLS = [
  ...
  { k: 'pyeong', t: '평' },
  { k: 'expNet', t: '기대월순수익', tip: '실제 예약률을 반영한 한 달 순수익. 렌트 단기임대로 돌렸을 때 실제로 손에 남는 돈 — 이 값이 클수록 좋아요(핵심 지표).' },
  ...
  return (
    <div>
      ...
      <div style={{ background: '#eff6ff', border: '1px solid #dbeafe', borderRadius: 9, padding: '9px 13px', margin: '0 0 10px', fontSize: 13, color: '#1e3a5f', lineHeight: 1.55 }}>
        ...
        <b role="button" style={{ color: '#4321F3', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => setShowTerms(true)}>용어 설명 보기</b>
        {demo ? '' : ' · 매물을 누르면 상세'}.
      </div>
              ...
              <tr>
                {COLS.map((c) => (
                  <th key={c.k} className={`col-${c.k}${c.l ? ' l' : ''}`} onClick={() => onSort(c.k)}>
                    {c.t}
                    {c.tip && (
                      <span role="button" title="설명 보기" style={{ color: '#4321F3', fontWeight: 700, cursor: 'help' }}
